...
class LuontoParser:
    ...
    def parse(path, title_in_html=""):
        ...
        if path == "/":
            first_card = grid_column.find('div', attrs={'class': 'diks-card diks-card--scrollable-image'})
            perm_link = first_card.find('a', attrs={'class': 'diks-card__link'})['href']
            return LuontoParser.parse(perm_link, title_in_html)

        perm_link = path
        this_card = grid_column.find('a', href=path, attrs={'class': 'diks-card__link'}).find_parent('div', attrs={'class': 'diks-card diks-card--scrollable-image'})
        older_card = this_card.find_next_sibling('div', attrs={'class': 'diks-card diks-card--scrollable-image'})
        newer_card = this_card.find_previous_sibling('div', attrs={'class': 'diks-card diks-card--scrollable-image'})
        next_link = None
        prev_link = None
